...
def pick(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    qualified_para = [para for para in paragraphs if select(para)]
    if k <= len(qualified_para) - 1:
        return qualified_para[k]
    else:
        return ''
    # END PROBLEM 1
...
def about(topic):
    ...
    def helper(para: str) -> bool:
        para_word_list = split(remove_punctuation(lower(para)))
...
def accuracy(typed, source):
    ...
    # BEGIN PROBLEM 3
    # pad short list with empty string
    m, n = len(typed_words), len(source_words)
    pad = ["" for _ in range(abs(m - n))]
    if m < n:
        source_words = source_words[0:m]
    else:
        source_words += pad
    str_length = len(source_words)
    return sum([1 if typed_words[ind] == source_words[ind] else 0 for ind in range(str_length)]) / str_length * 100
    # END PROBLEM 3 
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    """
    # BEGIN PROBLEM 5
    # typed_word is in word_list
    if typed_word in word_list:
        return typed_word
    else:
        diff_list = [(word, diff_function(typed_word, word, limit)) for word in word_list if diff_function(typed_word, word, limit) <= limit]
        if len(diff_list) == 0:
            return typed_word
        else:
            diff_list.sort(key=lambda x: x[1])
            return diff_list[0][0]
    # return lowest diff word below limit
    # if all lowest diff greater than limit, return typed_word
    # END PROBLEM 5
...
def minimum_mewtations(start, goal, limit):
    ...
    if limit < 0 :  # Fill in the condition
        ...
        # END
print(minimum_mewtations("bc", "adc", 20))
...
def time_per_word(words, times_per_player):
    ...
    # BEGIN PROBLEM 9
    n = len(words)
    def ts_2_time_per_word(timestamp_list):
        return [timestamp_list[i] - timestamp_list[i - 1] for i in range(1, n + 1)]
    
    times = [ts_2_time_per_word(ts) for ts in times_per_player]
...
def fastest_words(match):
    ...
    [4, 1, 6]
    """
    player_list = get_all_times(match)
    word_list = get_all_words(match)
    player_indices = range(len(player_list))  # contains an *index* for each player
    word_indices = range(len(word_list))    # contains an *index* for each word
    # BEGIN PROBLEM 10
    ans = [[] for _ in player_indices]
    for i in word_indices:
        fast_player_index = 0
        threshold = player_list[0][i]
        for j in player_indices:
            cur_speed = player_list[j][i]         
            if cur_speed < threshold:
                threshold = cur_speed
                fast_player_index = j                
        ans[fast_player_index].append(word_list[i])
    return ans
    # END PROBLEM 10
