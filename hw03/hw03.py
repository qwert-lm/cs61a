...
def pingpong(n):
    ...
    True
    """
    def pingpong_direction(n):
        if n <= 8:
            return 1
        elif (num_eights(n-1) > 0) or ((n-1) % 8 == 0):
            return (-1) * pingpong_direction(n-1)
        else:
            return pingpong_direction(n-1)    
    if n <= 8:
        return n
    else:
        return pingpong_direction(n) + pingpong(n - 1)
...
def count_coins(change):
    ...
    def helper(change, coin):
        if change == 0:
            return 1
        elif change < 0 or coin == None or (change < coin):
            return 0
        
        with_cur = helper(change - coin, coin)
        without_cur = helper(change, next_larger_coin(coin))
