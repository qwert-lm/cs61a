...
class TACard(Card):
    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        n = len(player.hand)
        if n > 0:
            hand_index = [i for i in range(n)]
            power_order = sorted(hand_index, key=lambda x: player.hand[x].power(opponent_card), reverse=True)
            best_card_index = power_order[0]
            best_card = player.hand[best_card_index]
            # add att and den
...
class InstructorCard(Card):
    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        -1000 -1000
        """
        self.attack -= self.decrease_num
        self.defense -= self.decrease_num
        if self.attack < 0 or self.defense < 0:
            re_add = False
        else:
            re_add = True
            player.hand.append(InstructorCard("Instructor Card", self.attack, self.defense))
        # You should add your implementation above this.
        if re_add:
