...
def play(strategy0, strategy1, update,
    ...
    while score0 < goal and score1 < goal:
        if not who:
            ...
            score0 = update(num_rolls=num_rolls,
                            player_score=score0,
            ...
            score1 = update(num_rolls=num_rolls,
                            player_score=score1,
...
def is_always_roll(strategy, goal=GOAL):
    ...
    """
    # BEGIN PROBLEM 7
    res_init = strategy(0, 0)
    for i in range(100):
        for j in range(100):
            if i == 0 and j == 0:
                pass
            else:
                res_temp = strategy(i, j)
                if res_temp != res_init:
                    return False
    return True
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    for roll_num in range(1, 11):
        score = make_averaged(roll_dice, total_samples)(roll_num, dice)
        if score > avg_score:
            avg_score = score
            best_num = roll_num
    return best_num
...
def tail_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    score_expect = tail_points(opponent_score=opponent_score)
    if score_expect >= threshold:
        return 0
    else:
        return num_rolls
    # END PROBLEM 10
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    score_expect = square_update(0, score, opponent_score)
    if score_expect - score >= threshold:
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
